...
export function createImageField({ host, directory, value, alt, canEdit, onBusy, onChange, kind = "photo", allowRemove = true }) {
  ...
  const imageAlt = () => typeof alt === "function" ? alt() : alt;
  const element = el("div", { className: "editor-image-field", role: "group", "aria-label": label });
  const image = el("img", { alt: imageAlt(), width: "160", height: "160" });
  image.hidden = true;
  ...
  remove.hidden = !allowRemove || (logo ? !value : !isCustomTeamPhoto(value));
  preview.append(image);
  element.append(el("p", {}, label), preview, currentPath, choose, ...(allowRemove ? [remove] : []), status);
  ...
  const show = (result) => {
    ...
    if (result?.ok && typeof result.previewUrl === "string" && /^(data:image\/(jpeg|png|webp);base64,|https?:\/\/)/i.test(result.previewUrl)) {
      image.src = result.previewUrl; image.alt = imageAlt(); image.hidden = false; preview.hidden = false; status.textContent = "";
