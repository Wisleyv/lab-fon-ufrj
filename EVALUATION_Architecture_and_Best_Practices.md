...
class ContentLoader {
  ...
  handleError(error) {
    ...
    const message =
      errorMessages[error.message] || "Erro ao carregar conteúdo.";
...
class HookSystem {
  ...
  apply(name, data) {
    ...
    return this.hooks[name].reduce((acc, hook) => hook.callback(acc), data);
  }
}
...
const STATIC_CACHE = ["/", "/css/main.css", "/js/main.js", "/data.json"];
